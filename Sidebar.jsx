  ...
  return (
    ...
    >
      <div className="p-4 border-b" style={{ borderColor: "var(--glass-border)" }}>
        ...
        <div className="space-y-2">
          ...
          <select
            ...
            onChange={(e) => onProjectChange({ ...project, info: { ...project.info, wilayaId: Number(e.target.value) } })}
            className="glass-input rounded-md px-2 py-1.5 text-sm w-full"
          >
            {WILAYAS.map((w) => (
              <option key={w.id} value={w.id} style={{ background: 'var(--app-bg-color)', color: 'var(--glass-text)' }}>{w.name}</option>
            ))}
          </select>
      ...
      <div className="flex-1 overflow-y-auto p-2">
        ...
        <button
          ...
          className={`w-full flex items-center gap-2 px-3 py-2.5 rounded-lg text-left transition mb-1 ${
            isProjectSummaryActive ? "bg-[var(--glass-accent-bg)] text-[var(--glass-primary)]" : "hover:bg-[var(--input-bg)] opacity-80 hover:opacity-100"
          }`}
        >
          ...
          return (
            <div key={local.id} className="mb-1">
              <div className="flex items-center gap-1">
                <button onClick={() => toggleLocal(local.id)} className="p-1 rounded hover:bg-[var(--input-bg)] opacity-50 hover:opacity-100">
                ...
                <button
                  ...
                  className={`flex-1 flex items-center gap-2 px-2 py-2 rounded-lg text-left transition ${
                    activeId?.type === "local" && activeId?.id === local.id ? "bg-[var(--glass-accent-bg)] text-[var(--glass-primary)]" : "hover:bg-[var(--input-bg)] opacity-80 hover:opacity-100"
                  }`}
                >
                  <Building2 className="h-4 w-4 shrink-0" />
                  <span className="font-medium truncate">{local.name}</span>
                </button>
                <button onClick={() => onDeleteLocal(local.id)} disabled={(project.locals ?? []).length <= 1} className="p-1 rounded hover:bg-[var(--danger-bg)] opacity-40 hover:text-[var(--danger-text)] disabled:opacity-20 transition">
                  <Trash2 className="h-4 w-4" />
                </button>
              </div>

              {isExpanded && rooms.map((room) => (
                  <div key={room.id} className="flex items-center gap-1 pl-6 pr-2 py-1">
                    <button
                      ...
                      className={`flex-1 flex items-center gap-2 px-2 py-2 rounded-lg text-left transition ${
                        activeId?.type === "room" && activeId?.localId === local.id && activeId?.roomId === room.id ? "bg-[var(--glass-accent-bg)] text-[var(--glass-primary)]" : "hover:bg-[var(--input-bg)] opacity-80 hover:opacity-100"
                      }`}
                    >
                      <DoorOpen className="h-4 w-4 shrink-0" />
                      <span className="font-medium truncate">{room.name}</span>
                    </button>
                    <button onClick={(e) => { e.stopPropagation(); onDeleteRoom?.(local.id, room.id); }} disabled={rooms.length <= 1} className="p-1 rounded hover:bg-[var(--danger-bg)] opacity-40 hover:text-[var(--danger-text)] disabled:opacity-20 transition">
                      <Trash2 className="h-4 w-4" />
                    </button>
                  </div>
                ))}
      ...
      <div className="p-2 border-t space-y-2" style={{ borderColor: "var(--glass-border)" }}>
        <button onClick={onAddLocal} className="glass-button-primary w-full flex items-center justify-center gap-2 px-3 py-2.5 rounded-lg text-sm font-semibold transition">
          <Plus className="h-4 w-4" /> Add Local
        </button>
        <button onClick={onAddRoom} className="glass-button w-full flex items-center justify-center gap-2 px-3 py-2.5 rounded-lg text-sm font-medium transition">
